...
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, '/blockparty.html'));
});
...
app.post('/highscore', async (req, res) => {
    ...
    for (let i = 0; i < obj.Contract.namedKeys.length; i++) {
        if(obj.Contract.namedKeys[i].name == "blockparty_highest_score") {
            const scoresData = await casperService.getBlockState(
                stateRootHash,
                hash,
                ['blockparty_highscore',obj.Contract.namedKeys[i].name]
            );
