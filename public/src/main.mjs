...
const accountHash = 'process.env.ACCOUNT_HASH';
...
btnSaveScore.addEventListener("click", async () => {
    ...
    usersHighestScore = document.getElementById('scoreValue').innerHTML;
    
    console.log(usersHighestScore);
...
function getUsersHighScore(hash){
    ...
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(JSON.stringify({
        value: 'process.env.HIGHSCORE_HASH',
        name: hash
    }));
    xhr.onload = function() {
